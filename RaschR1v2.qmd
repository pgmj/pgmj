...
-   You can practice using this file (RaschR1) as a template
    ...
    -   copy code, run the chunks, check the output, write commentary.
        **Repeat**
    -   *there is a file called "Exercise2solution" that shows how I did
        the analysis*
...
# them remove them from dataframe, since we need a dataframe with only item data
df.omit.na$Sex <- NULL
df.omit.na$Age <- NULL
...
RIitemcats(df.omit.na, items = c("q1","q5"))
...
Values in red are below/above the pre-set cutoff values (. We'll leave them for
now.
...
correlate more than the pre-set cutoff value of `r loc_dep` above the
