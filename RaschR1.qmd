...
- You'll run the setup code chunk, and practice navigation a bit
    + Enable/disable _Outline_
    + Navigate with Outline
    + Navigate with _chunks_
    + Switch between _source_ and _visual_ editor
    + Changing size of the quadrant windows
    + Look at the objects _df_ and _itemlabels_
:::
...
- You can practice using this file (RaschR1) as a template
    ...
    + _there is a file called "Exercise2solution" that shows how I did the analysis_
...
each item. The average correlation is calculated, and item pairs that correlate
...
(`r ldcut`). Looking at the item descriptions, both are related to
...
### Floor/ceiling effects - test parameters {.smaller}
