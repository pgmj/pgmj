...
The Perceived Stress Scale [@lee2012; @nordin2013] is a measure of subjective stress. It is available in three versions, with 14, 10 or 4 items. This dataset consists of the 10 item version, PSS-10. 

All items have the same response options, **`r responseOptions$ResponseText`**, which are recoded to 0-4 for this analysis. The questions are introduced with the text _**`r introText`**_.
...
We have `r nrow(df.omit.na)` participants and their gender distribution is shown in the table below. All participants are Swedish adults and the dataset was collected to validate a measure of social interactions at workplaces [@johansson2021; @johansson2021a].
...
Item 4 has issues with the second highest category, which will be merged with the category below.
...
We have multiple item pairs with problematic correlations:
...
Item 1 and 9 are still correlated above the cutoff (`r round(dyn.cutoff)`). Both items contain wording about being upset/angry about things that are unexpected or outside of one's control.
...
In our sample, there was DIF of age found between those with category 2 or lower (age 18-29), and those with category above 2 (age 30+).
...
itempar(pctree.out, node = c(2:3)) %>% # identify the nodes to compare (see plot above)
  ...
  t() %>%
  as.data.frame() %>%
  dplyr::rename('Age 18-29' = '2', # rename numerical categories to interpretable text
                'Age 30+' = '3') %>%
  rownames_to_column(var = "Item") %>%
  #mutate(Medel = rowMeans(.[2:3]), Stdev = rowSds(as.matrix(.[2:3]))) %>%
  mutate(Difference = .data[['Age 18-29']] - .data[['Age 30+']]) %>%
  mutate(across(where(is.numeric), round, 3)) %>%
  mutate(Item = str_remove_all(Item, "[difdata]")) %>%
...
#  row_spec(3:5, bold = T, color = "white", background = "lightblue") %>% 
  ...
  # latex_options are for PDF output
  kable_styling(latex_options = c("striped"))
  #formattable(list(table.attr = 'class=\"table table-striped\" style="font-size: 14px; font-family: Lato"'))
...
Significant DIF was found for items 3 and 9. This could be a good reason to remove item 9 instead of item 1 (residual correlation problems earlier). Our sample is really too small to draw conclusions, but since one of them needs to be removed, item 9 will be removed before moving on to the Targeting section.

However, there is another unique function in the Rasch Tree DIF, which is the interaction of multiple DIF variables. We can input age and gender at the same time! Our groups will be way too small, so this is really only to illustrate how to do it.
...
Still only age creating problems, which unfortunately means we won't get to make a neat table showing multiple groups, item locations, means and standard deviations.
...
ggplot(psimatrix) + 
  ...
  geom_hline(yintercept = 3.33, color = cutoff_line, linetype = 2, size = 0.7) +
...
Neither line is visible since test information doesn't reach 3.3 at any point.
...
It seems like the 5 remaining PSS items do not add up to a reliable scale. The peak test information value is `r round(peak.tif,2)`, and 3.33 is minimum (corresponding to reliability = 0.7).
